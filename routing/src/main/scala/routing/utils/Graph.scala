...
object Graph {
  ...
  def reload: ZIO[
    ...
    streets.clear()
    for {
      _ <- CrossroadRepository.findAllCrossroads.runCollect.map(points =>
        geoPoints ++= points.toList
      )
      _ <- BuildingRepository.findAllBuildings.runCollect.map(points =>
        geoPoints ++= points.toList
      )
  ...
  private def findGeoPointById(id: Int): GeoPoint =
    geoPoints.find(_.id == id).get

  def findGeoPointByIdSafe(id: Int): ZIO[Any, Throwable, GeoPoint] =
    geoPoints.find(_.id == id) match {
  ...
  private def addEdge(street: Street): Unit = {
    ...
    graphEdges(geoPointFrom.id) =
      Edge(geoPointFrom, geoPointTo, street.name, weight) +: graphEdges
        .getOrElse(geoPointFrom.id, Seq())
    graphEdges(geoPointTo.id) =
      Edge(geoPointTo, geoPointFrom, street.name, weight) +: graphEdges
        .getOrElse(geoPointTo.id, Seq())
  }
