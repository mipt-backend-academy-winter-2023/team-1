...
COPY --from=builder /workspace/helper/target/scala-2.13/*.jar .

EXPOSE 7777

ENTRYPOINT bash -c "source ~/.sdkman/bin/sdkman-init.sh && LC_ALL=en_DK.UTF-8 java -cp /workspace/*.jar helper.HelperMain"
