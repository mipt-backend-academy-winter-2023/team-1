...
COPY --from=builder /workspace/compressor/target/scala-2.13/*.jar .

EXPOSE 8088

ENTRYPOINT bash -c "source ~/.sdkman/bin/sdkman-init.sh && LC_ALL=en_DK.UTF-8 java -cp /workspace/*.jar compressor.CompressorMain"
