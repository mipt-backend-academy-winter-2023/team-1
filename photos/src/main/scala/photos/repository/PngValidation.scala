...
object PngValidation {
  ...
  private def checkHeader(
      buffer: Chunk[Byte]
  ): ZChannel[Any, Throwable, Chunk[Byte], Any, Throwable, Chunk[Byte], Any] = {
    ZChannel
      .readOrFail[NotPicture, Chunk[Byte]](NotPicture("is empty"))
      .flatMap { in =>
        ...
        if (prefix.length < magic.length) {
          ...
          ZChannel
            .write(prefix) *> ZChannel.identity[Throwable, Chunk[Byte], Any]
        } else {
